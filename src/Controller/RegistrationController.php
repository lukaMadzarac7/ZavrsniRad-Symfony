...
class RegistrationController extends AbstractController
{
    ...
    public function register(Request $request, UserPasswordHasherInterface $userPasswordHasher, UserAuthenticatorInterface $userAuthenticator, LoginFormAuthenticator $authenticator, EntityManagerInterface $entityManager, CountryRepository $countryRepository, UserRepository $userRepository): Response
    {
        $user = new User();
        ...
        if ($formUser->isSubmitted() && $formUser->isValid() && $formUserInfo->isSubmitted() && $formUserInfo->isValid()) {
            ...
            foreach ($users as $userTest){
                if($userTest->getUsername() == $formUser->get('username')->getData()){
